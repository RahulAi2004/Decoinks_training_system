...
// an AI-suggested rewrite) within 15 seconds before it is released to the agent.
...
function setPending(sessionId, { skipAgent }) {
  ...
  if (i < msgs.length && msgs[i].role === 'customer') {
    const m = msgs[i];
    db.prepare(`UPDATE real_chat_sessions SET pending_body = ?, pending_original = ?, pending_attachment = ?, pending_since = datetime('now'), next_index = ? WHERE session_id = ?`)
      .run(cleanBody(m.body), cleanBody(m.body), m.attachment_path || null, i + 1, sessionId);
    return 'pending';
  }
  db.prepare(`UPDATE real_chat_sessions SET pending_body = NULL, pending_original = NULL, pending_attachment = NULL, pending_since = NULL WHERE session_id = ?`).run(sessionId);
  return 'done';
}

export function createSupervised(agentId, realChatId) {
  ...
  db.prepare('INSERT INTO real_chat_sessions (session_id, real_chat_id, next_index, supervised) VALUES (?, ?, 0, 1)').run(id, chat.id);
...
export function pendingInfo(st) {
  ...
  return {
    ...
    seconds_left: Math.max(0, Math.ceil((HOLD_MS - elapsed) / 1000)),
  };
}
...
// Auto-release once the 15s hold elapses (checked whenever either side polls).
export function autoReleaseIfDue(sessionId) {
  const st = get(sessionId);
  if (!st?.pending_since) return;
